...
1. **If the script needs to be called from elsewhere** (another script, an automation, a dashboard button) — it must be a real script in `scripts.yaml`; a `sequence:` embedded directly in `mistral_tools.yaml` is not a registered entity and cannot be referenced anywhere else. This alone doesn't require a dedicated tool, though: if neither point 3 nor point 4 applies, simply exposing the script to Assist is enough for the LLM to call it too, via `execute_services`. A dedicated tool becomes necessary only once point 3 (input parameters) or point 4 (output response) comes into play.
